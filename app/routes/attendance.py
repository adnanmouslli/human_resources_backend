...
@token_required
def get_attendance_summary_by_date_range(user):
    ...
    try:
        ...
        # جلب جميع الموظفين المسموح للمستخدم برؤيتهم
        ...
        # جلب سجلات الحضور للموظفين المفلترين فقط في الفترة المحددة
        ...
        for attendance in attendances:
            ...
            # إصلاح استخراج التاريخ
            if hasattr(attendance.createdAt, 'date'):
                attendance_date = attendance.createdAt.date()
            else:
                # إذا كان attendance.createdAt عبارة عن date بالفعل
                attendance_date = attendance.createdAt
            
            if emp_id not in attendance_by_employee_date:
                attendance_by_employee_date[emp_id] = {}
            
            if attendance_date not in attendance_by_employee_date[emp_id]:
                attendance_by_employee_date[emp_id][attendance_date] = []
            
            attendance_by_employee_date[emp_id][attendance_date].append(attendance)

        result = []

        # معالجة كل موظف مفلتر في الفترة المحددة
        for employee in filtered_employees:
            # تحويل إلى date objects للمقارنة
            ...
            while current_date <= end_date_obj:
                # الحصول على سجلات الحضور لهذا الموظف في هذا التاريخ
                daily_attendances = attendance_by_employee_date.get(employee.id, {}).get(current_date, [])
                ...
                if daily_attendances:
                    ...
                    if filter_incomplete:
                        ...
                        # إذا كان الفلتر مُفعل، نعرض فقط السجلات الناقصة
                        if total_checkins > 0 and total_checkouts > 0 and total_checkins == total_checkouts:
                            # هذا سجل كامل، تجاهله إذا كان الفلتر للناقصين مُفعل
        ...
        print(f"ValueError in date range processing: {str(ve)}")
        return jsonify({'message': 'Invalid date format. Please use YYYY-MM-DD'}), 400
    except Exception as e:
...
def create_absence_summary_safe(employee, absence_date):
    """إنشاء ملخص للغياب مع معالجة آمنة للتواريخ"""
    try:
        # التأكد من أن absence_date هو date object
        if hasattr(absence_date, 'date'):
            absence_date = absence_date.date()
        ...
        # الحصول على أوقات الوردية المطلوبة
        required_start_time = None
        required_end_time = None
        
        if shift and employee.work_system == 'shift':
            is_working_day, required_start_time, required_end_time = get_shift_schedule_for_date(shift, absence_date)
            if not is_working_day:
                # يوم إجازة حسب الوردية
                return None
        
        # تجميع بيانات الموظف الكاملة
        employee_data = build_employee_data(employee)

        return {
            'employee': employee_data,
            'date': absence_date.isoformat(),
            'requiredCheckIn': str(required_start_time) if required_start_time else None,
            'requiredCheckOut': str(required_end_time) if required_end_time else None,
            'actualCheckIn': None,
...
def build_employee_data(employee):
    """بناء بيانات الموظف بشكل آمن"""
    try:
        employee_data = {
            'id': employee.id,
            ...
            'profession_id': getattr(employee, 'profession_id', None),
            'salary': float(employee.salary) if employee.salary else 0,
            'advancePercentage': float(employee.advancePercentage) if getattr(employee, 'advancePercentage', None) else None,
            'certificates': getattr(employee, 'certificates', None),
            ...
            'id_card_number': getattr(employee, 'id_card_number', None),
            'national_id': getattr(employee, 'national_id', None),
            'residence': getattr(employee, 'residence', None),
            'mobile_1': getattr(employee, 'mobile_1', None),
            'mobile_2': getattr(employee, 'mobile_2', None),
            'mobile_3': getattr(employee, 'mobile_3', None),
            'work_system': getattr(employee, 'work_system', None),
            'shift_id': getattr(employee, 'shift_id', None),
            'worker_agreement': getattr(employee, 'worker_agreement', None),
            'notes': getattr(employee, 'notes', None),
            'insurance_deduction': float(employee.insurance_deduction) if getattr(employee, 'insurance_deduction', None) else 0,
            'allowances': float(employee.allowances) if getattr(employee, 'allowances', None) else 0,
            'date_of_joining': employee.date_of_joining.isoformat() if getattr(employee, 'date_of_joining', None) else None,
            'created_at': employee.created_at.isoformat() if getattr(employee, 'created_at', None) else None,
            'updated_at': employee.updated_at.isoformat() if getattr(employee, 'updated_at', None) else None
        }

        # إضافة بيانات المسمى الوظيفي أو المهنة بشكل آمن
        ...
        print(f"Error building employee data for {employee.id}: {str(e)}")
        # إرجاع بيانات أساسية في حالة الخطأ
        return {
            'id': employee.id,
            'full_name': employee.full_name,
            'work_system': getattr(employee, 'work_system', 'hours')
        }


# استبدال استدعاء create_absence_summary بـ create_absence_summary_safe
def create_absence_summary(employee, absence_date):
    return create_absence_summary_safe(employee, absence_date)
